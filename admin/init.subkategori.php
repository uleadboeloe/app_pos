...
if(isset($_SESSION['ADMSESS_user_id'])){
    ...
    $ftp_file = "/MASTERBC/SubCategory.txt";
    $local_file = "fileupload/SubCategory.txt";
    ...
    /*
    ===============Tarik Flatfile Master Sub Kategori Daily BC=================================== 
    */
    ...
    if($Filesize > 0){
        ...
        for($x=0;$x<$CountArrays;$x++){
            ...
            if($StrLenKode ==6){
                $strQuery="SELECT * FROM dbo_subkategori WHERE kode_subkategori = '" . $Kode . "' and nama_subkategori = '" . $LabelName . "'";
                $callstrQuery=mysqli_query($koneksidb, $strQuery);
                $Jumbar=mysqli_num_rows($callstrQuery);
                if($Jumbar == 0){
                    $strInsert="INSERT INTO dbo_subkategori(`kode_subkategori`,`nama_subkategori`,`posting_date`,`posting_user`) VALUES ('$Kode','$LabelName','$datedb','" . $_SESSION['ADMSESS_user_id'] . "')";
