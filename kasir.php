...
$itemTableHeight = getConfig('ItemTableHeight');
//$valuePoin = getConfig('ValuePoin');
...
<head>
	...
	<style>
		...
		.topbtn {
			...
			border-radius: 5px;
			cursor: pointer;
			background-color:rgb(7, 112, 46);
		}
		.fontBig {
			font-size: 2.5em;
			font-weight: 500;
			color:rgb(209, 103, 32);
			padding:4px;
		}
		.qtybtn {
			border: none;
	...
	<style>
		...
		#scrollable thead th {
			position: sticky;
			top: 0;
			background: #edfff3;  /* penting agar header tidak transparan */
			z-index: 2;         /* agar tetap di atas isi tabel */
		}
        #listscanneditems {
            width: 100%;
            height: 100%;
	...
	<script>
		...
		scrollable.addEventListener("touchmove", function(e) {
			...
			// Cegah scroll body saat user menggeser di luar batas scrollable
			if ((atTop && direction > 0) || (atBottom && direction < 0)) {
			e.preventDefault(); // Ini hanya bekerja jika passive: false
			}
		}, { passive: false });
	</script>
	...
	<div id="wrapper">
        ...
        <div class="row-fluid" style="background-color: #4e8f64;">
            <div class="span8">
				<div>
					<!-- App Brand -->
					<div class="span4" style="border-right: 1px solid #9bb6cf; height: 50px;">
                		<p style="font-weight: 900; color: #fff; padding: 9px 0px 0px 15px; text-shadow: 1px 1px #253d2d;font-size: 140%;">InsanPOS <span style="font-size: 60%;">v.1.00</span></p>
					</div>
					<!-- Input Scan -->
					<div class="span8" style="padding-top: 8px;">
						<div class="span5" style="padding-top: 6px; text-align: right;">
							<span style="font-weight: 600; font-size: 115%; color: #fff; text-shadow: 1px 1px #253d2d;">No. :&nbsp;</span>
						...
						<div class="span7"><input id="EditBoxScanItem" type="number" placeholder="Scan item" class="span12" /></div>
					</div>
				</div>
            </div>
            <div class="span4" style="margin: 0 !important; width: auto !important; padding: 0 !important;">
				<!-- Header Buttons -->
				<div style="padding: 8px 10px 0px 10px; float: right;">
					<table>
						<tr>
							<td class="td-no-line-height"><span id="BtnSearch" class="btn btn-block btn-default btn-icon">🔍 SRC&nbsp;&nbsp;</span></td>
							<td class="td-no-line-height">&nbsp;</td>
							<td class="td-no-line-height"><span id="BtnAccount" class="btn btn-block btn-default btn-icon">📇 ACT&nbsp;&nbsp;</span></td>
							<td class="td-no-line-height">&nbsp;</td>
							<td class="td-no-line-height"><span id="BtnMember" class="btn btn-block btn-default btn-icon">Ⓜ️ MBR&nbsp;&nbsp;</span></td>
							<td class="td-no-line-height">&nbsp;</td>
							<td class="td-no-line-height"><span id="BtnLock" class="btn btn-block btn-default btn-icon">🔒 LCK&nbsp;&nbsp;</span></td>
							<td class="td-no-line-height">&nbsp;</td>
							<td class="td-no-line-height"><span id="BtnLogout" class="btn btn-block btn-default btn-icon">💵 STL&nbsp;&nbsp;</span></td>
						</tr>
					</table>
				</div>
            </div>
        ...
        <div id="content" style="background: #fff;"> 
			<div class="row-fluid">
				...
				<div class="span12" style="padding: 5px 5px 5px 5px;">
					<!-- MAIN TABLE -->


					<div id="scrollable" style="height: <?php echo $itemTableHeight . 'px;' ?>">
					...
					<table id="ItemTableSubTotal" class="table table-bordered table-condensed">
						<tr>
							<td style="text-align: right; margin-top:10px;">TOTAL ITEM</td>
							<td style="width: 100px; text-align: center;" class="fontBig" id="TxtTotalItems"><b>0</b></td>
							<td style="width: 70px; text-align: right; margin-top:10px;">SUBTOTAL</td>
							<td style="width: 274px; text-align: right;" class="fontBig" id="TxtSubTotal"><b>0</b></td>
						</tr>
					</table>
					<!-- PPN -->
					<table id="ItemTablePPN" class="table table-bordered" style="display:none;">
						<tr>
					...
					<!-- MEMBER POINT -->
					<table id="ItemTablePointMember" class="table table-bordered hidden" cellpadding="2px" cellspacing="2px">
						<tr>
							<td class="keypad-divider" style="display: flex; justify-content: flex-end; align-items: center;">
								<label style="display: flex; align-items: center; gap: 12px;">
									<span style="margin-top:10px;">POIN MEMBER</span>
									...
									<input type="checkbox" id="chkboxUsePoint" class="form-control" style="padding:4px;margin-top:10px;margin-right:10px;margin-left:10px;transform: scale(3);" checked>
								</label>
							</td>
							<td class="fontBig" id="TxtGrandTotalPoin" style="width: 278px; text-align: right; color:#FF0000"><b>0</b></td>
						</tr>
					</table>
					<!-- VOUCHER -->
					<table id="ItemTableVoucher" class="table table-bordered hidden" cellpadding="2px" cellspacing="2px">
						<tr>
							<td class="keypad-divider" style="display: flex; justify-content: flex-end; align-items: center;">
								<label style="display: flex; align-items: center; gap: 12px;">
									<span style="margin-top:10px;">VOUCHER</span>
									...
									<input type="checkbox" class="form-control" style="padding:4px;margin-top:10px;margin-right:10px;margin-left:10px;transform: scale(3);" checked disabled>
								</label>
							</td>
							<td class="fontBig" id="TxtVoucher" style="width: 278px; text-align: right; color:#FF0000"><b>0</b></td>
						</tr>
					</table>					
					<!-- PAYMENT -->	
					...
					<table id="ItemTableGrandTotal" class="table table-bordered table-condensed">
						<tr>
							<td class="fontBig" id="TxtGrandTotal" style="width: 70px; text-align: right; "><b>0</b></td>
						</tr>
					</table>	
				...
				<div id="TenderedPanel" class="span4" style="padding: 5px 10px 0px 0px; margin-left: 10px; pointer-events: none; opacity: 0.5; display: none;">
					<div class="widget">
...
<!-- Dialog Payment Cash -->	
<div id="DialogPaymentCash" title="Cash Payment"></div>
<script src="dialogbox/dlg_payment_cash.js"></script>
...
<!-- Dialog Edit Price -->	
<div id="DialogEditPrice" title="EDIT PRICE"></div>
