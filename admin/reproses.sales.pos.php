...
$NoStruk = isset($_GET['no_struk']) ? $_GET['no_struk'] : null;

$sql = "SELECT * FROM dbo_header WHERE no_struk = :no_struk";
$stmt = $db->prepare($sql);
$stmt->bindParam(':no_struk', $NoStruk);
$stmt->execute();
...
$sqlDetail = "SELECT * FROM dbo_detail WHERE no_struk =:no_struk";
$stmtDetail = $db->prepare($sqlDetail);
$stmtDetail->execute([':no_struk' => $NoStruk]);
...
// SYNC PAYMENT //
$sqlPayment = "SELECT * FROM dbo_payment WHERE no_struk =:no_struk order by noid ASC";
$stmtPayment = $db->prepare($sqlPayment);
$stmtPayment->bindParam(':no_struk', $NoStruk);
$stmtPayment->execute();
...
header("Location: sales-invoice-showmsg!save-failed@$NoStruk");
?>
